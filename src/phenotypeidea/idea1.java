...
public class idea1 extends GeneAndPhenotype {
	...
	public void inherit(Tree Girl,Tree Boy) {
		...
		for(int i=0;i<this.Gene.length;i++) {
			...
			} else this.Gene[i]=10*(Math.random()-0.5);
		}
	}
	...
	public void createBody(BodyPartsRegisterer register) {
		...
		createBranches(new Point(MyFrame.WindowW/2,MyFrame.WindowH-30),true,35,3.141592/2,10);
	}
	...
	private void createBranches(Point MeStartP,boolean sideDirection,double length,double MeAngle,int leftTime) {
		...
		double SideStartR=Math.abs(Gene[1])/(Math.abs(Gene[0])+Math.abs(Gene[1]));
		...
		double SideAngle=MeAngle+Gene[2]*(sideDirection?-1:1);//isSideLeftに応じて自分に対し右に行くか左に行くか
		
		//サイド
		createBranches(SideStartP, !sideDirection, 0.9*length, SideAngle, leftTime-1);
		//メイン
		createBranches(MeEndP, !sideDirection, 0.9*length, MeAngle, leftTime-1);
	}
}
